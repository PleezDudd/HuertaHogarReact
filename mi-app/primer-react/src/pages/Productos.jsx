...
export const Productos = () => {
  ...
  useEffect(() => {
    const cargarProductos = async () => {
      try {
        ...
        const response = await api.get("/api/productos?activos=true");
  ...
  // Lista de respaldo (solo si no hay productos del backend)
  ...
  // Normalizar productos del backend (adaptar campos si es necesario)
  const productosNormalizados = listaProductos.map((prod) => ({
    id: prod.id,
    nombre: prod.nombre || prod.name || "",
    categoria: prod.categoria || prod.category || "Sin categoría",
    precio: typeof prod.precio === "number" ? prod.precio : parsePrecio(prod.precio),
    stock: prod.stock || 0,
    descripcion: prod.descripcion || prod.description || "",
    imagen: prod.imagen || prod.image || "/img/default.png",
  }));

  const categorias = ["Todas", ...new Set(productosNormalizados.map((p) => p.categoria))];

  const productosFiltrados =
    categoriaSeleccionada === "Todas"
      ? productosNormalizados
      : productosNormalizados.filter((p) => p.categoria === categoriaSeleccionada);
      ...
      >
        {productosFiltrados.length === 0 ? (
          ...
          productosFiltrados.map((prod) => {
            ...
            const base = typeof prod.precio === "number" ? prod.precio : parsePrecio(prod.precio);
