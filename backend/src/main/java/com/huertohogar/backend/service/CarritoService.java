...
public class CarritoService {
    ...
    public Carrito agregarProducto(Long usuarioId, Long productoId, Integer cantidad) {
        ...
        if (itemExistente.isPresent()) {
            ...
            nuevoItem = itemCarritoRepository.save(nuevoItem);
            carrito.getItems().add(nuevoItem);
        }

        carrito = carritoRepository.save(carrito);
        // Refrescar el carrito desde la base de datos para asegurar que los items estén cargados
        // Usar el método que tiene @EntityGraph para cargar items y productos
        return carritoRepository.findByUsuarioIdAndActivoTrue(usuarioId)
                .orElseThrow(() -> new RuntimeException("Error al refrescar el carrito después de agregar producto"));
    }
    ...
    public Carrito getBoletaCompra(Long usuarioId) {
        // Obtener el carrito con items y productos cargados
        ...
        if (carrito.getItems() == null) {
            // Si los items no están cargados, cargarlos manualmente
            List<ItemCarrito> items = itemCarritoRepository.findByCarrito(carrito);
            carrito.setItems(items);
        }
        
        if (carrito.getItems().isEmpty()) {
            throw new RuntimeException("El carrito está vacío. No se puede generar la boleta.");
        }
        
        // Verificar que todos los items tengan productos cargados
